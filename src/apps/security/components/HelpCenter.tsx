...
export const HelpCenter: React.FC = () => {
  ...
  const handleSendTicket = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setStatusMessage({ text: "Lỗi khi gửi yêu cầu. Vui lòng thử lại.", type: 'error' });
    } finally {
  ...
  return (
    <div className="flex flex-col h-full bg-slate-950 rounded-[40px] overflow-hidden border border-slate-800 shadow-2xl relative">
      {/* Header */}
      <div className="bg-slate-900/80 backdrop-blur-md p-8 border-b border-slate-800 z-10">
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-8">
          <div className="space-y-2">
            <h2 className="text-3xl font-black tracking-tight text-white flex items-center gap-4  uppercase">
              <div className="w-12 h-12 bg-scmd-cyber/20 rounded-2xl flex items-center justify-center text-scmd-cyber shadow-lg shadow-scmd-cyber/10">
                <HelpCircle size={28} />
              </div>
              Trung tâm Hỗ trợ SCMD Pro
            </h2>
            <p className="text-slate-500 font-bold text-xs uppercase tracking-widest pl-1">Knowledge Base & Intelligence Service Center</p>
          </div>
          <div className="relative w-full lg:w-[480px] group">
            <Search className="absolute left-5 top-1/2 -translate-y-1/2 text-slate-500 group-focus-within:text-scmd-cyber transition-colors" size={20} />
            <input 
              type="text" 
              placeholder="Nhập từ khóa cần tra cứu (VD: Báo cáo, SOS, AI...)" 
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              className="w-full h-16 pl-14 pr-6 bg-slate-950 border border-slate-800 rounded-3xl text-sm font-black text-white focus:outline-none focus:border-scmd-cyber focus:ring-4 focus:ring-scmd-cyber/5 transition-all shadow-inner placeholder:text-slate-700"
            />
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="hidden md:block w-96 shrink-0 bg-[var(--color-bg)] border-r border-slate-800 overflow-y-auto p-8 space-y-10 custom-scrollbar">
                ...
                <div className="space-y-1.5">
                  {categoryArticles.map(article => (
                    <button
                      ...
                      className={cn(
                        "w-full text-left px-5 py-4 rounded-2xl text-sm font-semibold transition-all flex items-start gap-2 group tracking-normal leading-snug",
                        selectedArticle?.id === article.id 
                          ? "bg-scmd-cyber text-slate-950 shadow-xl shadow-scmd-cyber/20" 
          ...
          <div className="pt-10 border-t border-slate-800 mt-10">
            <div className="bg-slate-950 p-6 rounded-[32px] border border-slate-800 space-y-4 shadow-xl">
              ...
              <button 
                ...
                className="w-full py-3 bg-slate-900 border border-slate-800 text-scmd-cyber hover:bg-scmd-cyber hover:text-slate-950 rounded-xl text-[10px] font-black tracking-widest transition-all uppercase"
              >
                Gửi yêu cầu ngay
              </button>
            </div>
        ...
        {/* Content Area */}
        <div className="flex-1 bg-slate-950 overflow-y-auto p-8 lg:p-16 custom-scrollbar relative">
          {selectedArticle ? (
            <div className="max-w-4xl mx-auto space-y-12 animate-in fade-in slide-in-from-bottom-8 duration-700">
              ...
              <div className="prose prose-invert prose-lg max-w-none 
                ...
                bg-slate-900/50 p-12 lg:p-16 rounded-[48px] border border-white/10 shadow-2xl backdrop-blur-xl relative group">
                  ...
                  <>
                    <div className="space-y-6">
                      <div className="space-y-3">
                        <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-2">Vấn đề cần hỗ trợ</label>
                        <SCMDInput 
                          required
                      ...
                      <div className="space-y-3">
                        <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-2">Nội dung chi tiết</label>
                        <textarea 
                          required
