...
const ROLE_MAP: Record<string, { label: string; color: string; icon: React.ReactNode }> = {
  ...
  'vendor-commander': {
    label: 'VENDOR CMD',
    color: 'bg-orange-500/10 text-orange-400 border-orange-500/20 shadow-[0_0_10px_rgba(249,115,22,0.1)]',
    icon: <BadgeCheck size={10} />,
  },
  'vendor-representative': {
    label: 'VENDOR REP',
    color: 'bg-teal-500/10 text-teal-400 border-teal-500/20 shadow-[0_0_10px_rgba(20,184,166,0.1)]',
    icon: <UserCog size={10} />,
  },
  guard: {
    label: 'SECURITY',
...
export const getStatusInfo = (status?: string, isOnline?: boolean) => {
  ...
  if (status === 'inactive')
    return { label: 'NGOẠI TUYẾN', color: 'text-scmd-silver/40', dot: 'bg-scmd-silver/40' };
  if (status === 'suspended')
    return { label: 'TẠM DỪNG', color: 'text-amber-400', dot: 'bg-amber-500' };
  return { label: 'VẮNG MẶT', color: 'text-scmd-silver/20', dot: 'bg-scmd-navy' };
};

export const getDisplayStaffCode = (s: Pick<Staff, 'staffId' | 'id'>): string => {
  const normalizedStaffId = typeof s.staffId === 'string' ? s.staffId.trim() : '';
  if (normalizedStaffId) return normalizedStaffId.toUpperCase();

  const normalizedId = typeof s.id === 'string' ? s.id.trim() : '';
  return normalizedId ? normalizedId.toUpperCase() : '—';
};
