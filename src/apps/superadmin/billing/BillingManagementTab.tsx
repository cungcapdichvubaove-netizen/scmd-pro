    ...
    <div className="space-y-6 animate-in fade-in">
      ...
      {error ? (
        <EmptyState
          ...
          title="Không thể tải dữ liệu billing"
          description={error}
          action={(
            <button
              type="button"
              onClick={() => setRefreshKey(k => k + 1)}
              disabled={loading}
              className="h-12 px-6 rounded-xl bg-scmd-admin-rail border border-white/5 text-[#8892B0] hover:text-white disabled:opacity-50 disabled:cursor-not-allowed transition-all font-bold"
            >
              Thử tải lại
            </button>
          )}
