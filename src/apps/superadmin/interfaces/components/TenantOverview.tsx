    ...
    <div className="space-y-10 animate-in fade-in duration-700">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        <div className="md:col-span-2 space-y-8">
          {/* Growth Chart */}
          <div className="bg-scmd-admin-panel border border-white/[0.05] rounded-xl p-8 shadow-2xl">
            <div className="flex justify-between items-start mb-8">
              <div>
                <div className="flex items-center gap-2 text-scmd-cyber mb-1">
                  <TrendingUp size={16} />
                  <span className="text-xs font-semibold uppercase text-[#4285F4]">{t('dashboard.growth_velocity')}</span>
                </div>
              ...
              <div className="flex gap-8">
                <div className="text-right">
                  ...
                  <p className="text-xs font-semibold text-scmd-admin-text-muted uppercase">Hàng ngày</p>
                </div>
                <div className="text-right">
                  <p className="text-3xl font-black text-white">+{stats?.growthVelocity?.weekly ?? 0}</p>
                  <p className="text-xs font-semibold text-scmd-admin-text-muted uppercase">Hàng tuần</p>
                </div>
              </div>
            </div>
          ...
          <div className="bg-scmd-admin-panel border border-white/[0.05] rounded-xl p-8 shadow-2xl">
            <div className="flex items-center gap-3 mb-6">
              ...
              <div>
                ...
                <p className="text-xs text-scmd-admin-text-muted font-medium">Hệ thống tự động nhận diện Lead tiềm năng.</p>
              </div>
            </div>
            <div className="space-y-4">
              {stats?.smartNotifications?.length === 0 ? (
                <div className="py-10 text-center border border-dashed border-white/10 rounded-2xl">
                  <p className="text-scmd-admin-text-muted text-sm font-medium">Chưa có thông báo mới.</p>
                </div>
                ...
                <div key={notif.id} className={cn(
                  ...
                  <div className="flex-1">
                    <div className="flex justify-between items-start mb-1">
                      ...
                      <span className="text-xs font-semibold text-scmd-admin-text-muted/80 uppercase">{new Date(notif.timestamp).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
                    </div>
                    <p className="text-xs text-scmd-admin-text-muted leading-relaxed mb-3">{notif.message}</p>
                    <button 
                      onClick={() => setContactLead(notif)}
                      className="px-4 py-2 bg-scmd-cyber text-white rounded-xl font-semibold text-xs uppercase hover:bg-[#4285F4] transition-all"
                    >
        ...
        <div className="space-y-8">
          <div className="bg-scmd-admin-panel border border-white/[0.05] rounded-xl p-8 shadow-2xl relative overflow-hidden group">
            <div className="flex items-center gap-2 text-scmd-safety mb-4">
              <Server size={16} />
              <span className="text-xs font-semibold uppercase text-[#10B981]">System Architecture</span>
            </div>
            <div className="space-y-4">
              <div className="flex items-center justify-between p-3 bg-white/[0.03] rounded-xl border border-white/5">
                ...
                <span className="text-xs font-semibold text-scmd-safety uppercase">Active</span>
              </div>
              <div className="flex items-center justify-between p-3 bg-white/[0.03] rounded-xl border border-white/5">
                <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-semibold text-scmd-safety uppercase">Running</span>
              </div>
            </div>
          </div>

          <div className="bg-scmd-admin-panel border border-white/[0.05] rounded-xl p-8 shadow-2xl relative overflow-hidden">
            <div className="flex items-center gap-2 text-amber-400 mb-4">
              <DollarSign size={16} />
              <span className="text-xs font-semibold uppercase">{t('dashboard.revenue_stream')}</span>
            </div>
            <div className="flex justify-between items-end">
              <div>
                <p className="text-4xl font-black text-white">${stats?.revenueStream?.totalRevenue?.toLocaleString() ?? 0}</p>
                <p className="text-xs font-semibold text-scmd-admin-text-muted uppercase mt-1">Tổng doanh thu</p>
              </div>
      ...
      {/* Whale Alerts Table */}
      <div className="bg-scmd-admin-panel border border-white/[0.05] rounded-xl p-10 shadow-2xl">
        <div className="flex justify-between items-center mb-10">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-scmd-admin-text-muted font-medium">Top {t('entities.tenants')} tiềm năng cao dựa trên quy mô nhân sự & mục tiêu.</p>
            </div>
          </div>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="text-scmd-admin-text-muted/80 text-xs uppercase border-b border-white/5">
                ...
                <tr key={whale.id} className="group hover:bg-white/5 transition-colors">
                  <td className="py-6">
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 bg-scmd-admin-sidebar rounded-xl flex items-center justify-center text-scmd-admin-text-muted group-hover:scale-110 transition-transform">
                        <Building2 size={20} />
                      </div>
                  ...
                  <td className="py-6">
                    <div className="flex flex-col">
                      ...
                      <span className="text-xs font-semibold text-scmd-admin-text-muted uppercase">{whale.checkpointCount} điểm</span>
                    </div>
                  </td>
                  <td className="py-6">
                  ...
                  <td className="py-6">
                    <span className="text-xs font-semibold text-amber-500 uppercase">{whale.potentialValue} POTENTIAL</span>
                  </td>
                  <td className="py-6 text-right">
                    <button 
                      onClick={() => setContactLead(whale)}
                      className="px-6 py-2 bg-scmd-cyber text-white rounded-xl font-semibold text-xs uppercase hover:bg-[#4285F4] transition-all"
                    >
                      {t('common.contact_now')}
                    </button>
                  </td>
