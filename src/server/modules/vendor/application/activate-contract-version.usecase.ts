...
export class ActivateContractVersionUseCase extends BaseUseCase<{ contractId: string; versionId: string }, any> {
  ...
  protected async internalExecute(context: SecurityContext, request: { contractId: string; versionId: string }): Promise<any> {
    const result = await VendorRepository.activateContractVersion(context, request.contractId, request.versionId);
    await AuditService.log({
      userId: context.userId,
      tenantId: context.tenantId,
      action: 'CONTRACT_VERSION_ACTIVATE',
      resource: `contracts/${request.contractId}/versions/${request.versionId}`,
      payload: { archivedPreviousVersionId: result.archivedPrevious?.id || null },
      status: 'SUCCESS',
    });
    return result;
  }
